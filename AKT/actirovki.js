document.addEventListener('DOMContentLoaded', function () {
  ...
  const getWeatherForecastBtn = document.getElementById(
    'getWeatherForecastBtn'
  );
    ...
    getWeatherForecastBtn.addEventListener('click', function () {
      fetch('/get-weather-forecast/')
        ...
        .then((data) => {
          ...
          if (data.error) {
            ...
            if (Array.isArray(data.data.weather_data)) {
              ...
              console.error(
                'Ожидаемые данные о погоде не найдены или не являются массивом'
              );
            }
          }
        })
        .catch((error) => {
          ...
          alert(
            'Произошла ошибка при получении данных. Подробности в консоли.'
          );
        });
    });
  }
  ...
  document
    .getElementById('regenerateTableBtn')
    .addEventListener('click', function () {
      let dataToSend = {
        ...
        'Poriv do':
          parseFloat(document.getElementById('porivdo').value) || null,
      };
  ...
  document
    .getElementById('updateForecastBtn')
    .addEventListener('click', function () {
      ...
      fetch('/izmenit/', {
        ...
        .then((response) => {
          ...
          document.querySelector('img.img-rem').src = response.image_path
            ? response.image_path
            : defaultImagePath;
        })
        .catch((error) => console.error('Ошибка:', error));
  ...
  button.addEventListener('click', function () {
    ...
    if (!isOn) {
      // Если текущее состояние было "Включить", значит мы включаем систему
  ...
  function sendSwitchStateUpdate() {
    const isOn = button
      .querySelector('.text')
      .textContent.includes('Выключить');
    const data = {
